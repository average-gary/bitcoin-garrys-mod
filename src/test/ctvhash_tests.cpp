    ...
    for (unsigned int idx = 0; idx < tests.size(); idx++) {
        ...
        if (test.isStr())
            ...
            for (size_t i = 0; i < hash.size(); ++i) {
                ...
                if (sh != hash[i]) {
                    BOOST_ERROR("Expected: " << sh.GetHex() << " Got: " << hash[i].GetHex() << " For:\n"
                                             << strTest);
                }
            }
                ...
                for (auto use_random_index = 0; use_random_index < 2; ++use_random_index) {
                    ...
                    for (size_t i = 0; i < hash.size(); ++i) {
                        ...
                        if (hash_will_change && hash_equals) {
                            BOOST_ERROR("Expected: NOT " << hash[i].GetHex() << " Got: " << sh.GetHex() << " For:\n"
                                                         << strTest);
                        } else if (!hash_will_change && !hash_equals) {
                            BOOST_ERROR("Expected: " << hash[i].GetHex() << " Got: " << sh.GetHex() << " For:\n"
                                                     << strTest);
                        }
                    }
                }
