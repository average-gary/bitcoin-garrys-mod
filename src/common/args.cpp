...
std::variant<ChainType, std::string> ArgsManager::GetChainArg() const
{
    ...
    if ((int)chain_arg.has_value() + (int)fRegTest + (int)fSigNet + (int)fTestNet + (int)fTestNet4 + (int)fTestNet5 > 1) {
        throw std::runtime_error("Invalid combination of -regtest, -signet, -testnet, -testnet4, -testnet5 and -chain. Can use at most one.");
    }
    if (chain_arg) {
