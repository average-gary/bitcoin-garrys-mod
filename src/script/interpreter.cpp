...
static bool EvalChecksigFromStack(const valtype& sig, const valtype& msg, const valtype& pubkey_in, ScriptExecutionData& execdata, script_verify_flags flags, SigVersion sigversion, ScriptError* serror, bool& success)
{
    /*
...
#define FLAG_NAME(flag) {std::string(#flag), SCRIPT_VERIFY_##flag}
    static const std::map<std::string, script_verify_flag_name> g_names_to_enum{
        ...
        FLAG_NAME(CHECKTEMPLATEVERIFY),
        FLAG_NAME(DISCOURAGE_UPGRADABLE_CHECKTEMPLATEVERIFY),
        FLAG_NAME(DISCOURAGE_CHECKTEMPLATEVERIFY),
